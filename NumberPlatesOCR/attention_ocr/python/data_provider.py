...
def random_resize_method(image, height, width):
    resize_methods = [
    ...
    method_index = tf.random.uniform([], maxval=len(resize_methods), dtype=tf.int32)
    selected_method = tf.switch_case(method_index, branch_fns={
        0: lambda: tf.image.resize(image, [height, width], method=tf.image.ResizeMethod.BILINEAR),
        1: lambda: tf.image.resize(image, [height, width], method=tf.image.ResizeMethod.NEAREST_NEIGHBOR),
        2: lambda: tf.image.resize(image, [height, width], method=tf.image.ResizeMethod.BICUBIC),
        3: lambda: tf.image.resize(image, [height, width], method=tf.image.ResizeMethod.AREA)
    })    
...
def augment_image(image):
  ...
  distorted_image = random_resize_method(distorted_image, height, width)
