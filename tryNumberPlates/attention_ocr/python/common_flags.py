...
def define():
  ...
  flags.DEFINE_string('dataset_dir', './tryNumberPlates/attention_ocr/python/datasets/testdata/fsns', 'Dataset root folder.')
...
def create_dataset(split_name):
    ...
    dataset = ds_module.get_split(split_name, dataset_dir=FLAGS.dataset_dir)

    

    def generator():
        data_provider = slim.dataset_data_provider.DatasetDataProvider(
            dataset, common_queue_capacity=20, common_queue_min=10)
        while True:
    ...
    print('tf_dataset type', type(tf_dataset))

    # Assuming these attributes are provided by the dataset module
    # num_char_classes = ds_module.num_char_classes
    # max_sequence_length = ds_module.max_sequence_length
    # num_of_views = ds_module.num_of_views
    # null_code = ds_module.null_code
